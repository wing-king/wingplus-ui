...
import "./picker.less";
// Utils
import { pick, extend, unitToPx, isSameValue, preventDefault, BORDER_UNSET_TOP_BOTTOM, type Numeric } from "../utils";
import {
	bem,
	name,
	isOptionExist,
...
// Composables
import { useChildren, useEventListener, useParent } from "@wingplus-ui/use";
import { useExpose } from "../composables/use-expose";

// Components
import { Loading } from "../loading";
import Column, { PICKER_KEY } from "./PickerColumn";
import Toolbar, { pickerToolbarPropKeys, pickerToolbarSlots } from "./PickerToolbar";
import { PickerColumn, PickerExpose, PickerOption, pickerProps } from "./types";
...
export default defineComponent({
	...
	setup(props, { emit, slots }) {
		...
		const onClickOption = (currentOption: PickerOption, columnIndex: number) =>
			emit("clickOption", extend({ columnIndex, currentOption }, getEventParams()));
		...
		const renderColumnItems = () =>
			currentColumns.value.map((options, columnIndex) => (
				<Column
					...
					onClickOption={(option: PickerOption) => onClickOption(option, columnIndex)}
				/>
			));
